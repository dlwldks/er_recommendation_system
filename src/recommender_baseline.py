...
from src.scoring import compute_distance_and_time_features, normalize_features
from src.explanation import generate_explanation


def _sort_baseline_candidates(
    candidates_df: pd.DataFrame,
    baseline_type: str,
) -> pd.DataFrame:
    working_df = candidates_df.copy()

    if baseline_type == "distance_only":
        working_df["final_score"] = -working_df["distance_km"]
        working_df = working_df.sort_values(
            by=["distance_km", "estimated_wait_min", "hospital_score"],
            ascending=[True, True, False],
        ).reset_index(drop=True)

    elif baseline_type == "total_time_only":
        working_df["final_score"] = -working_df["total_time_min"]
        working_df = working_df.sort_values(
            by=["total_time_min", "distance_km", "hospital_score"],
            ascending=[True, True, False],
        ).reset_index(drop=True)
...
def recommend_er_baseline(
    ...
    ranked = _sort_baseline_candidates(
...
def recommend_for_all_patients_baseline(
