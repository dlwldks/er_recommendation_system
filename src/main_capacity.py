...
from src.recommender_capacity import recommend_for_all_patients_with_capacity
from src.visualization import plot_recommendation_scatter


def main() -> None:
    paths = Paths()
    model_name = "capacity_aware"
    ...
    results_df = recommend_for_all_patients_with_capacity(
        patients_df=patients_df,
        hospitals_df=hospitals_df,
        symptom_map_df=symptom_map_df,
        top_k=TOP_K,
        sort_by_severity_first=True,
    )
    ...
    display_df["final_score"] = pd.to_numeric(display_df["final_score"], errors="coerce").round(6)
    ...
    print("\n=== Capacity-Aware Recommendation Results (Summary) ===")
